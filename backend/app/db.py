...
def init_db():
    with _connect() as c:
        ...
        c.execute(
            "CREATE TABLE IF NOT EXISTS assessments (id INTEGER PRIMARY KEY, user_id INTEGER, created_at TEXT NOT NULL, data_json TEXT NOT NULL, result_json TEXT NOT NULL)"
        )
...
def save_assessment(user_id, data, result):
    with _connect() as c:
        cur = c.execute(
            "INSERT INTO assessments(user_id,created_at,data_json,result_json) VALUES(?,?,?,?)",
            (
                user_id,
...
def list_assessments(user_id=0, limit=30):
    with _connect() as c:
        rows = c.execute(
            "SELECT id, created_at, result_json FROM assessments WHERE user_id=? ORDER BY created_at DESC LIMIT ?",
            (user_id, limit),
        ).fetchall()
