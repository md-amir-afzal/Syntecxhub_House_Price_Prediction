...
"severe breathing difficulty": r"(?:can't\s*breathe|cannot\s*breathe|severe\s*breathing|breath(?:ing)?\s*(?:is\s*)?very\s*difficult|saans\s*(?:nahi|nahin)\s*(?:aa|le)|saans\s*lene\s*(?:mein|me)\s*(?:mushkil|dikkat)|सांस\s*(?:नहीं|बहुत)\s*(?:आ|ले)|सांस\s*लेने\s*(?:में|मे)\s*(?:मुश्किल|दिक्कत))",
...
"severe abdominal pain": r"(?:severe\s*abdominal\s*pain|severe\s*stomach\s*pain|pet\s*(?:mein|me)\s*bahut\s*dard|पेट\s*(?:में|मे)\s*बहुत\s*दर्द)",
...
def apply_safety(analysis:dict,source_text:str)->dict:
    ...
    if flags:
        analysis.update({"risk_level":"Emergency","emergency":True,"doctor_required":True,"red_flags":flags,"recommended_action":EMERGENCY_MESSAGE,"otc_information":[],"self_care":["Do not delay emergency medical care."],"safety_note":"Emergency warning overrides routine self-care advice."})
    else:
        analysis["emergency"]=False; analysis.setdefault("red_flags",[]); analysis.setdefault("safety_note","Educational triage information only. This is not a diagnosis or prescription.")
    return analysis
